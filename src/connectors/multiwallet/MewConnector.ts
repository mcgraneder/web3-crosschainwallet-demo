import { EthAddress } from "@renproject/chains-ethereum-old";
...
export class EthereumMEWConnectConnector extends AbstractEthereumConnector<MewProvider> {
  ...
  activate: ConnectorInterface<MewProvider, EthAddress>["activate"] = async () => {
  ...
  getProvider = async () => {
    ...
    const { Provider } = await import("@myetherwallet/mewconnect-web-client").then((m) =>
